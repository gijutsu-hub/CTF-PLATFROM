...
function Emaildomain($email, $con){
    if( filter_var( $email, FILTER_VALIDATE_EMAIL ) ) {
    // split on @ and return last value of array (the domain)
    $domain = array_pop(explode('@', $email));
    }
    $sql = "select * from veriemails where domains='$domain'";  
...
function adduser($username, $email, $password, $verifi, $con){
    $query    = "INSERT into users (userid, email, pass, veri)VALUES ('$username','$email', '" . md5($password) . "',  '$verifi')";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
    ...
    if($user) {
        $domveri = Emaildomain($email, $con);
    }else{
        $_SESSION['errors'] = array("This email/userid is already being used, or use your official email id");
        header("location: Register.php");
    }
}else{
    $_SESSION['errors'] = array("Recapture Verification failed");
    header("location: Register.php");
}
    if($domveri){
    ...
    if($result){
        $result1 = adduser($username, $email, $password, $verifi, $con);
    }
    else{
        $_SESSION['errors'] = array("This email/userid is already being used, or use your official email id ");
        header("location: Register.php");
...
</tr><tr><td align="center" class="es-m-txt-c" style="padding:0;Margin:0;padding-bottom:10px"><h1 style="Margin:0;line-height:46px;mso-line-height-rule:exactly;font-family:arial, "helvetica neue", helvetica, sans-serif;font-size:46px;font-style:normal;font-weight:bold;color:#333333">'.$verifi.'</h1></td>
</tr><tr><td align="center" class="es-m-p0r es-m-p0l" style="Margin:0;padding-top:5px;padding-bottom:5px;padding-left:40px;padding-right:40px"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, "helvetica neue", helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px">Once confirmed, this email will be uniquely associated with your account.</p></td></tr></table></td></tr></table></td></tr></table></td></tr></table></td></tr></table></div></body></html>';
        if($mail->send()){
