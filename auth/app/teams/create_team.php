...
function checkteam($tname, $con){
  $sql = mysqli_query($con,"SELECT * FROM team WHERE tname='$tname'");
...
$email = email($username, $con);
  if($user){
    if(!empty($_POST['name'])){
        ...
        $result = checkteam($name, $con);
        if($result){
            $sql = "INSERT into team (tname, tid, userid, tlogo, tdesc)VALUES ('$name','$id', '$username',  '$logo', '$desc')";
            $result   = mysqli_query($con, $sql);
            if ($result) {
              mysqli_query($con,"INSERT into tpoints (tid, tname) VALUES ('$id', '$name')");
              $msg = "Team created successfully and team id ".$id."<br> <a href = '../profile/profile.php'> Go to Profile </a>" ;
              $mail->addAddress("$email");
              $mail->Subject = 'Team Registration';
    ...
    <section>
      <form action="" method="post" class="login-form">
        <div class="input-group">
          <label for="verification">Team Name</label>
          <input type="text" placeholder="Team Name" name="name" required>
        </div>
        <div class="input-group">
          <label for="verification">Team logo url</label>
          <input type="text" placeholder="https://example.com/logo.png" name="logo" required>
        </div>
        <div class="input-group">
          <label for="verification">Team description</label>
          <input type="text" placeholder="Eat, sleep, Hack and repeat" name="desc" required>
        </div>
        <div class="input-group" ><button>Create Team</button></div>
      </form>
    </section>
    <footer>
    <div class="input-group">
