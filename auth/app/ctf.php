...
include('session.php');
include('../conn.php');
$sql = mysqli_query($con,"SELECT DISTINCT * FROM   ctfch WHERE  challengeid NOT IN (SELECT challengeid FROM solve WHERE username = '$username');");
$sql1 = mysqli_query($con,"SELECT * FROM evedetails");
$row1 = mysqli_fetch_array($sql1, MYSQLI_ASSOC);
$sql2 = mysqli_query($con,"SELECT * FROM ctfch where verified  = 'TRUE'");
...
<div class="app-container">
  ...
  <h1>Hackme if you can</h1>
    ...
    <div class="chart-container-wrapper small">
        <div class="chart-container">
          ...
          <p class="profile-text">Total Solve</p>
          </div>
        </div>
  <?php
  ...
  <div class="chart-container-wrapper small">
    ...
    <a href ="ctfdetails.php?id=<?php echo base64_encode($row["challengeid"]); ?> ">
    <?php }else{ ?>
      <a href ="#">
    <?php } ?>
        <div class="chart-container">
          <div class="chart-container-header">
            <h3><?php echo $row["ctfname"]?></h3>
          </div>
          <div class="chart-container-header">
          ...
          <div class="chart-container-header">
            <h3><?php echo $row["point"]?></h3>
          </div>
          <div class="chart-container-header">
            <h3><?php 
            $cid = $row["challengeid"];
            $sql3 = mysqli_query($con,"SELECT * FROM solve where username  = '$username' AND challengeid ='$cid' ");
            if($sql3->num_rows > 0){ 
              echo "Solve";
            }else{
              echo "NOT SOLVE";
              $_SESSION['solve'] = array("NOT SOLVE TILL");
            }
             ?></h3>
          </div>
          <p class="profile-text"><?php echo $row["solve"]?></p>
          </div>
</a>
        </div>
        <?php
